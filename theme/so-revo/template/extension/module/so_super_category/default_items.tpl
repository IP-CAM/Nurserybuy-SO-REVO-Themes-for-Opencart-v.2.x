...
if (!empty($child_items)) {
	...
	foreach ($child_items as $item) {
		...
		<div class="ltabs-item products-list grid">
        <?php }?>
        	...
        	<div class="product-layout">
        		<div class="product-item-container">
        			<div class="left-block so-quickview">
        				...
        				<a class="hidden" data-product='<?php echo $item['product_id'];?>' href="<?php echo $item['link'] ?>"  title="<?php echo $item['name'] ?>" target="<?php echo $item_link_target ?>">				</a>
					...
					<div class="box-label">
						...
						<?php if ($item['special'] && $display_sale) : ?>
									<span class="label-product label-sale"><?php echo $item['discount']; ?></span>
								<?php endif; ?>
					...
					<div class="right-block">
						...
						<div class="button-group">
							...
							{
							?>
								<button class="addToCart" type="button" onclick="cart.add('<?php echo $item['product_id']; ?>');"><span><?php echo $objlang->get('button_cart'); ?></span></button>
							<?php }?>
							<?php if($display_wishlist == 1){
							?>
								<button class="wishlist btn-button" type="button" data-toggle="tooltip" title="<?php echo $objlang->get('button_wishlist'); ?>" onclick="wishlist.add('<?php echo $item['product_id']; ?>');"><i class="fa fa-heart"></i></button>
							<?php }?>
							<?php if($display_compare == 1){
							?>
								<button class="compare btn-button" type="button" data-toggle="tooltip" title="<?php echo $objlang->get('button_compare'); ?>" onclick="compare.add('<?php echo $item['product_id']; ?>');"><i class="fa fa-exchange"></i></button>
							<?php }?>
						</div>
					</div>
